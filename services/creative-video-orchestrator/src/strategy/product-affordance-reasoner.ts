...
export class ProductAffordanceReasoner {
  ...
  /**
   * Resolves physical product affordance, natural operating environment,
   * human actor roles, and cross-sector negative constraints.
   *
   * Queries existing Omnistudio ChatGPT service when available;
   * seamlessly falls back to offline semantic ontology if gateway is unreachable.
   * Zero external API key required.
   */
  public async reasonAffordance(query: AffordanceQuery): Promise<ProductAffordanceReport> {
    const { brandName = '', productName = '', productDescription = '' } = query

    // 1. Try querying existing ChatGPT service via OmniStudio gateway
    try {
      const resp = await fetch(`${this.gatewayUrl}/v1/chat/affordance`, {
        method: 'POST',
      ...
      if (resp.ok) {
        ...
        if (data && data.naturalEnvironment && Array.isArray(data.negativeEnvironmentConstraints)) {
          return {
            detectedSector: data.detectedSector || 'commercial_product',
            naturalEnvironment: data.naturalEnvironment,
            environmentKeywords: data.environmentKeywords || [],
            actorRole: data.actorRole || 'Kullanıcı',
            primaryProductAction: data.primaryProductAction || 'Ürün kullanımı',
            lightingProfile: data.lightingProfile || 'Doğal gün ışığı',
            negativeEnvironmentConstraints: data.negativeEnvironmentConstraints,
            confidence: typeof data.confidence === 'number' ? data.confidence : 0.95,
            source: 'chatgpt_service',
          }
        }
      }
    } catch {
      // Gateway offline or timed out: fall back seamlessly to deterministic semantic rules
    }

    // 2. Deterministic Semantic Fallback (Zero network / zero key reliance)
    return this.deduceSemanticAffordance(query)
  }

  public deduceSemanticAffordance(query: AffordanceQuery): ProductAffordanceReport {
    const { brandName = '', productName = '', productDescription = '' } = query
    ...
    if (
      text.match(/(oto|araç|arac|araba|lastik|jant|motor|servis|ekspertiz|bakım|bakim|detailing|kaplama|tamir)/)
    ) {
      return {
        detectedSector: 'automotive_service',
        naturalEnvironment:
          'Pırıl pırıl, aydınlık ve profesyonel otomotiv detailing stüdyosu ve servis plazasında dinamik çekim',
...
export const productAffordanceReasoner = new ProductAffordanceReasoner()
