...
export function QuietLink({
  ...
  return (
    <Link href={href} prefetch className={cx(buttonBase, buttonVariants.quiet, className)}>
      {children}
    </Link>
  )
}
...
const STATUS_STYLES: Record<string, { label: string; tone: string; hint?: string }> = {
  ...
  failed: { label: 'Hata Oluştu', tone: 'text-danger border-danger/35 bg-danger/10', hint: 'Gönderilemedi; ayrıntı satırda' },
  stopped: { label: 'İptal Edildi', tone: 'text-danger border-danger/35 bg-danger/10', hint: 'Elle veya kısıt nedeniyle durdu' },
