...
export function CampaignBulkReply({
  ...
  const fetchAiForSingle = async (targetId: number, lastMessage: string, phone: string) => {
    ...
    try {
      const res = await fetch('/api/mesajlar/ai-suggest', {
        ...
        body: JSON.stringify({ lastMessage, phone }),
      })
  ...
  const handleGenerateAllAi = async () => {
    ...
    setStatusNotice({
      tone: 'accent',
      text: 'Yapay zeka motoru yanıtları canlı olarak üretiyor...',
    })
    ...
    setStatusNotice({
      tone: 'accent',
      text: 'Tüm yanıtlar yapay zeka tarafından başarıyla güncellendi.',
    })
  }
        ...
        action={
          <div className="flex items-center gap-2">
            <Button
              ...
              title="Tüm soruları AI motoruna sorarak yanıtları yeniler"
            >
              {isBulkAiGenerating ? 'AI Üretiyor…' : 'AI Yanıtlarını Yenile'}
            </Button>
            <Button
                  ...
                  {item.cat === 'opt_out' ? (
                    ...
                    <div>
                      <div className="mb-1 flex items-center justify-between text-[11px] text-ink-faint">
                        ...
                        <div className="flex items-center gap-2">
                          {isItemAiLoading ? (
                            <span className="text-accent animate-pulse">AI yazıyor…</span>
                          ) : (
                            <button
                              type="button"
                              onClick={() => fetchAiForSingle(item.target_id, item.last_inbound_text, item.phone_e164)}
