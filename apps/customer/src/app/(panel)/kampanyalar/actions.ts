...
export async function updateCampaign(
  ...
  const startMode = String(formData.get('start_mode') ?? '').trim()
  const scheduledAtRaw = String(formData.get('scheduled_at') ?? '').trim()
  ...
  const validationError =
    ...
    validateMediaUrl(mediaUrl) ||
    validateAbSettings(abPercent, bodyB) ||
    (startMode === 'schedule' ? validateSchedule(startMode, scheduledAtRaw) : null)
  if (validationError) return { error: validationError }
  if (!name) return { error: 'Kampanyaya bir ad verin.' }
  ...
  const { data: existing, error: loadError } = await supabase
    .from('campaigns')
    .select('id, status, source_list_ids, scheduled_at')
    .eq('id', campaignId)
    .eq('org_id', org.id)
    .maybeSingle()
  ...
  if (existing.status === 'draft' && startMode === 'now') {
    const { error: jobError } = await enqueueJob({
      type: 'campaign.start',
      campaignId,
      priority: 10,
    ...
    if (jobError) {
      revalidatePath(`/kampanyalar/${campaignId}`)
      revalidatePath('/kampanyalar')
      return { error: 'Kaydedildi; gönderim başlatılamadı. Detaydan Başlat’ı deneyin.' }
    }
    revalidatePath(`/kampanyalar/${campaignId}`)
    revalidatePath('/kampanyalar')
...
/** Seçili gruplardaki benzersiz telefon sayısı (kara liste hariç). */
export async function countUniqueRecipients(
  listIds: string[],
  ...
  try {
    ...
    return { count: 0, error: error instanceof Error ? error.message : 'Oturum yok' }
  }
  ...
  for (;;) {
    const { data, error } = await supabase
      ...
      .select('contacts!inner(phone_e164)')
      .eq('org_id', org.id)
      .in('list_id', ids)
    ...
    for (const row of rows) {
      const nested = row.contacts as { phone_e164?: string } | { phone_e164?: string }[] | null
      const phone = Array.isArray(nested) ? nested[0]?.phone_e164 : nested?.phone_e164
  ...
  const { data: blocked } = await supabase
    ...
    .select('phone_e164')
    .eq('org_id', org.id)
  for (const row of blocked ?? []) phones.delete(row.phone_e164)
