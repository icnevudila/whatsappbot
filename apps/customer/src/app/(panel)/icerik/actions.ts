  ...
  if (productIds.length > 0) {
    ...
    for (const id of productIds) {
      ...
      products.push({
        ...
        detailImageUrl: (extra as any)?.detailImageUrl?.trim() || null,
        productKeyFeature: (extra as any)?.productKeyFeature?.trim() || null,
        logoPlacement: (extra as any)?.logoPlacement?.trim() || 'flat_rigid_surface',
