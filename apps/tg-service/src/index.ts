...
import { CHANNEL, env, hasDatabase, loadTgConfig } from './env.js'
import { enqueueChannelJob, handleChannelJob } from './job-handlers.js'

const logger = createWorkerLogger('tg-service', env.workerId)
const drainState = createDrainState()
const startedAt = Date.now()

export type TgHealth = HealthSnapshot & {
  degraded?: boolean
  draining?: boolean
  db?: boolean
  worker?: string
...
export async function getHealth(): Promise<TgHealth> {
...
export { env, loadTgConfig, drainState }
