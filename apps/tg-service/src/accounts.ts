...
function ensureMockDefault(): ChannelAccountRow {
  ...
  const row: ChannelAccountRow = {
    ...
    label: 'mock-telegram',
    status: 'connected',
    credentials: env.token ? { bot_token: env.token, token: env.token } : {},
    metadata: {},
  }
...
export async function listConnectedTelegramAccounts(): Promise<ChannelAccountRow[]> {
  const db = getDb()
  if (!db || !hasDatabase) {
...
export function tokenFromCredentials(credentials: Record<string, unknown>): string {
  const token =
    (typeof credentials.bot_token === 'string' && credentials.bot_token) ||
