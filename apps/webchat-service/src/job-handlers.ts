...
import { env, loadWebchatConfig } from './env.js'
...
async function handleSend(job: ChannelJobRow): Promise<unknown> {
  ...
  const token =
    (typeof payload.token === 'string' && payload.token) || credToken || env.token
  ...
  if (!result.ok) {
    throw new Error(result.error || 'webchat_send_failed')
  }
  return result
}
...
async function handleWebhookProcess(job: ChannelJobRow, db: DbHelpers | null): Promise<unknown> {
  ...
  const event = parseInbound(raw, { orgId, accountId })
  if (!event) {
