...
import { CHANNEL, env, hasDatabase, loadWebchatConfig } from './env.js'
import { enqueueChannelJob, handleChannelJob } from './job-handlers.js'

const logger = createWorkerLogger('webchat-service', env.workerId)
const drainState = createDrainState()
const startedAt = Date.now()

export type WebchatHealth = HealthSnapshot & {
  degraded?: boolean
  draining?: boolean
  db?: boolean
  worker?: string
...
export async function getHealth(): Promise<WebchatHealth> {
    ...
    try {
      const rows = await db.query<{ count: string }>(
        `select count(*)::text as count from public.channel_jobs
          where status = 'pending' and channel = 'webchat'`,
      )
...
export { env, loadWebchatConfig, drainState }
