...
import { clearThreads } from './adapter.js'
...
test('handle channel.send with mock send', async () => {
  ...
  const job: ChannelJobRow = {
    ...
    channel: 'webchat',
    type: 'channel.send',
    payload: { threadId: 'w1', text: 'merhaba' },
    attempts: 1,
    max_attempts: 3,
  }
  ...
  assert.match(r.externalMessageId ?? '', /^mock-webchat-/)
})
...
test('handle channel.webhook.process parseInbound', async () => {
  ...
  const job: ChannelJobRow = {
    ...
    payload: {
      raw: { text: 'selam', threadId: 'w2', senderId: 'u' },
    },
    attempts: 1,
    max_attempts: 3,
  }
  ...
  assert.equal(r.event.externalThreadId, 'w2')
})

test('handle unknown type throws NonRetryableJobError', async () => {
  const job: ChannelJobRow = {
    id: '3',
    org_id: null,
