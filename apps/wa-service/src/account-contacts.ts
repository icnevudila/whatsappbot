...
export async function persistAccountContacts(
  ...
  for (const c of contacts) {
    ...
    validRows.push({ phone, jid, name, notify })
  }
  ...
  for (let i = 0; i < validRows.length; i += BATCH_SIZE) {
    ...
    // Parametreleri hazirla: ($1, $2, $3, $4, $5, $6), ($7, ...
  ...
  log.info({ accountId, count: inserted }, 'WhatsApp kisileri account_contacts tablosuna kaydedildi')
  return inserted
}
...
export async function importAccountContactsToList(options: {
  ...
  accountId: string
  listName?: string
}): Promise<{ imported: number; listId: string; listName: string }> {
  ...
  // 2) account_contacts tablosundan ve message_log'daki numaralardan tum kisileri topla
  const contacts = await query<{ phone_e164: string; wa_jid: string; name: string | null }>(
    `with source_numbers as (
       select phone_e164, wa_jid, coalesce(name, notify) as name
        ...
        where account_id = $1
       union
       select distinct phone_e164, remote_jid as wa_jid, null as name
         from public.message_log
        where account_id = $1 and phone_e164 is not null
     )
     select phone_e164, wa_jid, max(name) as name
       from source_numbers
      where phone_e164 ~ '^\\+[1-9][0-9]{7,14}$'
      group by phone_e164, wa_jid`,
    [accountId],
  )

  if (contacts.length === 0) {
    log.info({ accountId }, 'Ice aktarilacak WhatsApp kisi kaydi bulunamadi')
    return { imported: 0, listId: list.id, listName: list.name }
  }
  ...
  for (let i = 0; i < contacts.length; i += BATCH_SIZE) {
    ...
    // contacts tablosuna ekle / guncelle (wa_status: 'valid')
    ...
    for (const c of batch) {
      ...
      contactPlaceholders.push(`($${idx + 1}, $${idx + 2}, $${idx + 3}, $${idx + 4}, 'whatsapp', 'valid', $${idx + 5}, now())`)
    }
    ...
    if (contactIds.length > 0) {
      // contact_list_members baglantisi
  ...
  // Liste eleman sayisini guncelle
  await query(
  ...
  log.info(
    { accountId, listId: list.id, count: linkedCount },
    'WhatsApp rehberi basariyla kisi listesine aktarildi',
  )

  return { imported: linkedCount, listId: list.id, listName: list.name }
}
