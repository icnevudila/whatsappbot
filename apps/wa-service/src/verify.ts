...
/** Bu org'a ait canli bir oturum bulur; dogrulama bagli bir hesap ister. */
export function findLiveSessionForOrg(orgId: string): WhatsAppSession | undefined {
  return sessionManager.liveSessions().find((session) => session.orgId === orgId)
}
...
export async function verifyContacts(
  ...
  const session = findLiveSessionForOrg(orgId)
  if (!session) {
    throw new Error('Dogrulama icin bagli bir WhatsApp hesabi gerekiyor')
  }

  const rows = await selectPending(orgId, payload)
  if (rows.length === 0) {
    return { checked: 0, valid: 0, invalid: 0 }
  }

  log.info({ orgId, count: rows.length }, 'Numara dogrulamasi basliyor')
  ...
  log.info({ orgId, ...summary }, 'Numara dogrulamasi bitti')
  return summary
}
...
async function selectPending(
  ...
  if (payload.list_id) {
    return query<PendingContact>(
      ...
      [orgId, payload.list_id, staleBefore],
    )
  }
