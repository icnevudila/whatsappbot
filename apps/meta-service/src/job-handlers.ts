...
import { parseInbound, parseMetaWebhook, sendMessage } from './adapter.js'
import {
  loadChannelAccount,
  pageIdFromCredentials,
  tokenFromCredentials,
} from './accounts.js'
import { env, loadMetaConfig, resolveMetaJobChannel } from './env.js'
...
async function handleSend(job: ChannelJobRow): Promise<unknown> {
  ...
  const pageId =
    ...
    (account ? pageIdFromCredentials(account.credentials, account.metadata) : '') ||
    env.pageId

  const channel =
    job.channel === 'instagram' || job.channel === 'facebook'
      ? job.channel
      : resolveMetaJobChannel(payload)
...
async function handleWebhookProcess(job: ChannelJobRow, db: DbHelpers | null): Promise<unknown> {
  ...
  const events = parseMetaWebhook(raw, { orgId, accountId })
  if (events.length === 0) {
    const single = parseInbound(raw, { orgId, accountId })
    if (!single) {
