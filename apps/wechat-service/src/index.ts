...
import { CHANNEL, env, hasDatabase, loadWechatConfig } from './env.js'
import { enqueueChannelJob, handleChannelJob } from './job-handlers.js'

const logger = createWorkerLogger('wechat-service', env.workerId)
const drainState = createDrainState()
const startedAt = Date.now()

export type WechatHealth = HealthSnapshot & {
  degraded?: boolean
  draining?: boolean
  db?: boolean
  worker?: string
...
export async function getHealth(): Promise<WechatHealth> {
    ...
    try {
      const rows = await db.query<{ count: string }>(
        `select count(*)::text as count from public.channel_jobs
          where status = 'pending' and channel = 'wechat'`,
      )
...
export { env, loadWechatConfig, drainState }
