...
import { CHANNEL, env, hasDatabase, loadLineConfig } from './env.js'
import { enqueueChannelJob, handleChannelJob } from './job-handlers.js'

const logger = createWorkerLogger('line-service', env.workerId)
const drainState = createDrainState()
const startedAt = Date.now()

export type LineHealth = HealthSnapshot & {
  degraded?: boolean
  draining?: boolean
  db?: boolean
  worker?: string
...
export async function getHealth(): Promise<LineHealth> {
    ...
    try {
      const rows = await db.query<{ count: string }>(
        `select count(*)::text as count from public.channel_jobs
          where status = 'pending' and channel = 'line'`,
      )
...
export { env, loadLineConfig, drainState }
