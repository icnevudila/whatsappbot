...
function buildSteps(progress: Progress): Step[] {
  ...
  const s3 = progress.counts.outCount > 0
  ...
  return [
    ...
    {
      n: 3,
      title: 'İlk mesajı dene',
      body: 'Kendine kısa bir test yaz (Tek numara) veya gruba kampanya başlat.',
      href: '/kampanyalar#hizli',
      cta: 'Test gönder',
      done: s3,
      current: s1 && s2 && !s3,
    },
...
/** Üye için net 3 adım — menüyü kilitlemez. */
        ...
        <div className="flex flex-wrap items-start justify-between gap-2">
          <div className="min-w-0">
            ...
            <h2 className="mt-0.5 text-[15px] font-bold tracking-[-0.02em] text-ink">
              {allGuideDone ? 'Kurulum tamam' : '3 adımda gönderime hazır ol'}
            </h2>
            <p className="mt-1 text-[12.5px] leading-snug text-ink-muted">
              {allGuideDone
                ? 'Hat, kişiler ve ilk gönderim hazır. İstersen Yardım’dan detaya bak.'
                : `${doneCount}/3 bitti · Sıradaki: ${current?.title ?? '—'}`}
            </p>
          </div>
          {variant === 'card' ? (
...
export function SetupBanner({ progress }: { progress: Progress }) {
  if (!progress.steps.connected) {
    return (
      <div className="mb-3 rounded-md border border-accent/25 bg-accent-soft/40 px-3.5 py-3">
        <p className="text-[13px] font-semibold text-ink">1 / 3 · WhatsApp hattını bağla</p>
        <p className="mt-1 text-[12.5px] leading-snug text-ink-muted">
          QR veya telefon koduyla bağla. Durum “Bağlı” olunca 2. adıma geçersin.
        </p>
        <div className="mt-2 flex flex-wrap gap-3">
          <AccentLink href="/hesaplar">Hat bağla →</AccentLink>
          <QuietLink href="/kurulum">Tüm adımlar</QuietLink>
        </div>
      </div>
    )
  ...
  if (!progress.steps.contacts) {
    return (
      <div className="mb-3 rounded-md border border-accent/25 bg-accent-soft/40 px-3.5 py-3">
        <p className="text-[13px] font-semibold text-ink">2 / 3 · Kişi grubu ekle</p>
        <p className="mt-1 text-[12.5px] leading-snug text-ink-muted">
  ...
  if (progress.counts.outCount === 0) {
    return (
      <div className="mb-3 rounded-md border border-hairline bg-surface px-3.5 py-3">
        <p className="text-[13px] font-semibold text-ink">3 / 3 · İsteğe bağlı test</p>
        <p className="mt-1 text-[12.5px] leading-snug text-ink-muted">
          Kendine kısa bir mesaj at — hat ve numara çalışıyor mu gör.
        </p>
        <div className="mt-2 flex flex-wrap gap-3">
          <AccentLink href="/kampanyalar#hizli">Test gönder →</AccentLink>
          <QuietLink href="/kurulum">Tüm adımlar</QuietLink>
        </div>
      </div>
    )
